...
/* ---------- LIHAT DI TANGGAL LAIN — "tanggal simulasi" opsional yang
   ...
   PIN, dan nanti teks epilogue/gift/farewell/final di slide-slide
   berikutnya) tetap otomatis mengikuti tanggal & jam ASLI perangkat —
   getEffectiveNow() cuma jadi "pass-through" ke new Date(). Begitu ada
   tanggal manual yang dipilih & diterapkan, dateOverride diisi dan
   seluruh fungsi siklus (getBirthdayCycleState, getNextSeptemberFirstWIB,
   updatePinRequestCountdown) otomatis ikut tanggal itu tanpa reload.
   TIDAK menyentuh: playlist (urutan lagu sudah final sejak IS_BIRTHDAY_TODAY
   di atas dihitung saat halaman pertama dimuat) dan tema siang/malam
   (applyTimeOfDayTheme tetap pakai jam asli perangkat). ---------- */
...
function applyLetterPhoto() {
  ...
  if (!imgEl) return;
  const isEpilogue = getBirthdayCycleState() === "epilogue";
  if (isEpilogue) {
...
function openLetter() {
  playWipeTransition(() => {
    ...
    if (isEpilogue) {
      if (dateEl) dateEl.textContent = "buat hari ini, hari apa pun itu ✦";
...
function goGift() {
  showScreen("gift");
  giftTheme = giftCopy(getBirthdayCycleState() === "epilogue");
...
function applyCycleTheme(state) {
  ...
  if (state === "epilogue") {
    if (eyebrow) eyebrow.textContent = "STILL HERE ✦";
    if (heading) heading.innerHTML = "Belum waktunya,<br><em>tapi tetap boleh mampir.</em>";
    if (sub) sub.textContent = "Ceritanya nggak berhenti di satu hari doang ✦ kalau kamu udah punya PIN, tetap bisa masuk kapan aja.";
    if (reqEyebrow) reqEyebrow.textContent = "THE STORY CONTINUES";
    if (reqCopy) reqCopy.innerHTML = "Momennya udah lewat, tapi halaman ini nggak berhenti di situ. Hitung mundur berikutnya baru mulai lagi <b>30 hari</b> sebelum 1 September.";
    fireworksMode = "off";
  } else if (state === "countdown-close") {
    if (eyebrow) eyebrow.textContent = "ALMOST THERE ✦";
    if (heading) heading.innerHTML = "Tinggal hitungan hari,<br><em>siap-siap ya.</em>";
    if (sub) sub.textContent = "Makin deket harinya ✦ kalau ini beneran buat kamu, kamu pasti udah tau angkanya.";
    if (reqEyebrow) reqEyebrow.textContent = "SO CLOSE NOW";
    if (reqCopy) reqCopy.innerHTML = "Tinggal seminggu lagi menuju <b>1 September</b>. Kalau sudah punya PIN yang benar, kamu tetap bisa masuk kapan saja.";
    fireworksMode = "hype";
  } else if (state === "countdown-early") {
    if (eyebrow) eyebrow.textContent = "COUNTDOWN BEGINS ✦";
    if (heading) heading.innerHTML = "Masukkan PIN,<br><em>kalau kamu tahu.</em>";
    if (sub) sub.textContent = "Kalau ini beneran buat kamu, kamu pasti tahu angkanya ✦";
...
/* ---------- KARTU "LIHAT DI TANGGAL LAIN" — panel kecil di bawah kartu
   PIN. Defaultnya OTOMATIS (dateOverride null, ikut tanggal asli
   perangkat). Kalau tanggal manual dipilih & "Terapkan" ditekan,
   dateOverride diisi lalu updatePinRequestCountdown() (yang sudah
   jalan tiap detik lewat setInterval di bawah) otomatis menuliskan
   ulang badge/eyebrow/heading/countdown dalam waktu ≤1 detik — tidak
   perlu reload halaman. Format tanggal simulasi dianggap tengah malam
   WIB, konsisten dengan CONFIG.birthday. ---------- */
...
function initDatePreview() {
  const toggle = $("#datePreviewToggle");
  const panel = $("#datePreviewPanel");
  const input = $("#datePreviewInput");
  const applyBtn = $("#datePreviewApplyBtn");
  const resetBtn = $("#datePreviewResetBtn");
  const status = $("#datePreviewStatus");
  if (!toggle || !panel || !input || !applyBtn || !resetBtn || !status) return;

  function renderStatus() {
    if (dateOverride) {
      status.innerHTML = `Melihat sebagai: <b>${formatDatePreviewLabel(dateOverride)}</b>`;
      resetBtn.hidden = false;
    } else {
      status.textContent = "Otomatis ✦ mengikuti tanggal hari ini";
      resetBtn.hidden = true;
    }
  }

  // Default nilai input = tanggal aktif saat ini (override kalau ada,
  // kalau tidak tanggal asli perangkat), biar gampang dari titik itu.
  const wibNow = getWibParts(getEffectiveNow());
  input.value = `${wibNow.year}-${String(wibNow.month).padStart(2, "0")}-${String(wibNow.day).padStart(2, "0")}`;
  renderStatus();
  ...
  applyBtn.addEventListener("click", () => {
    ...
    const picked = new Date(val + "T00:00:00+07:00");
    if (isNaN(picked)) return;
    dateOverride = picked;
    renderStatus();
    updatePinRequestCountdown(); // langsung tulis ulang, jangan tunggu tick berikutnya
  });

  resetBtn.addEventListener("click", () => {
    dateOverride = null;
    const realNow = getWibParts(new Date());
    input.value = `${realNow.year}-${String(realNow.month).padStart(2, "0")}-${String(realNow.day).padStart(2, "0")}`;
    renderStatus();
...
function updatePinRequestCountdown() {
  ...
  if (state === "epilogue") {
    ...
    el.innerHTML = months >= 1
      ? `masih sekitar <b>${months}</b> bulan lagi menuju cerita berikutnya ✦`
      : `menuju cerita berikutnya, pelan-pelan ✦`;
    return;
  }
